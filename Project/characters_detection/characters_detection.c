	...
	for(size_t i = 0; i < *nbChars; i++)
	{
		...
		for(size_t j = offset; j < size - offset; j++)
		{
			...
			for(size_t k = offset; k < size - offset; k++)
			{
				double value = 0;
				size_t count = 0;
				for (size_t l = (size_t)row; l > (size_t)row-(size_t)factorRow; l--){
					for (size_t m = col; m > (size_t)col-(size_t)factorCol; m--){
						if (l > 0 && m > 0){
							value += (double)image->data[l][m].R;
							count++;
						}
					}
				}
				value = value / count <= 128 ? 0 : 255;
				RGB rgb = {(uint8_t)value, (uint8_t)value, (uint8_t)value};
				tab[i]->data[j][k] = rgb;
				col += factorCol;
			}
			row += factorRow;
